...
export class Hud {
  ...
  constructor(world: World, opts: HudOptions = {}) {
    ...
    this.pipDot = el("span", { class: "dot" });
    this.valueEl = el("span", { class: "hud__value", textContent: "0" });

    this.segmentsEl = el("div", {
      class: "segments",
      aria: { role: "progressbar", valuemin: 0, valuemax: segments, valuenow: 0, label: "Lesson progress" },
    });
    ...
    this.element = el(
      ...
      opts.showPips === false
        ...
        : el(
            "div",
            { class: "hud__chip", aria: { label: "Pips collected", live: "polite" } },
            this.pipDot,
            this.valueEl
          )
    );
  ...
  /** Call once mounted so collected pips fly to the right place. */
  registerPipTarget(): void {
    requestAnimationFrame(() => {
      const r = this.pipDot.getBoundingClientRect();
      this.world.pipTarget = { x: r.left + r.width / 2, y: r.top + r.height / 2 };
    });
  }

  setPips(value: number, animate = true): void {
