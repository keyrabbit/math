 ...
 *    per biome), which pre-baked samples cannot do.
 *
...
class AudioEngine {
  ...
  /** A star lighting up in a constellation. */
  starLight(index = 0): void {
  ...
  /** Constellation completed — the big moment. An arpeggio that resolves upward. */
  fanfare(): void {
  ...
  /** Pips being collected — a quick ascending flurry. */
  ...
  /**
   * A slow, barely-there drone bed that gives each biome its own emotional colour. Two detuned
