...
interface Particle {
  ...
  size: number;
  color: string;
  kind: "sprinkle" | "flour" | "ring" | "trail";
  rotation: number;
  spin: number;
  gravity: number;
  drag: number;
  /** Optional homing target — sprinkles fly to the jar counter rather than just fading. */
  target?: { x: number; y: number };
  homing: number;
}

/** A jar of sprinkles is never one colour. */
 ...
 * everything the game needs: drifting flour in the air (the room feels alive), sprinkles that
 * *home* toward the HUD jar (reward feels earned and connects cause to effect), expanding rings
 * (impact), and trails.
 */
export class Particles {
  ...
  /** Celebration burst — used on correct answers and when a treat comes out of the oven. */
  burst(x: number, y: number, opts: { count?: number; color?: string; speed?: number; target?: { x: number; y: number } } = {}): void {
    ...
    for (let i = 0; i < count; i++) {
      ...
      this.push({
        ...
        color: SPRINKLE_COLORS[i % SPRINKLE_COLORS.length] ?? color,
        kind: "sprinkle",
  ...
  /** Sprinkles that fly to a HUD target. The homing ramps up so they arc rather than beeline. */
  ...
  /** Slow drifting flour. Called every frame; internally rate-limited. */
  ...
  update(dt: number): void {
    for (let i = this.items.length - 1; i >= 0; i--) {
      ...
      // Flour drifts sideways on a slow sine so it never falls in straight lines.
      if (p.kind === "flour") {
  ...
  draw(c: DrawContext): void {
    ...
    for (const p of this.items) {
      ...
      switch (p.kind) {
        ...
        case "flour": {
          // Fade in and out so flour never pops into existence.
        ...
        default: {
          // Sprinkles are little tumbling capsules, not sparkles — the same motion reads as
          // "shaken out of a jar" rather than "magic".
          const alpha = 1 - t * t;
          const size = p.size * (1 - t * 0.35);
          glow(ctx, p.x, p.y, size * 4.5, p.color, alpha * 0.5);
          ctx.save();
